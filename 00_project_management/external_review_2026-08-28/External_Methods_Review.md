...
**Status: feedback received; qualified methodological closure and author reapproval pending.**
...
Their originals are archived under `received/` with a copy-integrity manifest.
...
The author confirmation form is `04_submission/Author_Confirmation.md` in the
repository and `governance/Author_Confirmation.md` in the review ZIP. Both authors
must identify the exact reviewed package hash when confirming. A final journal,
matching new DOI, final file hashes and upload authorization remain separate
decisions. No agent-created signature or generic continuation request closes them.
