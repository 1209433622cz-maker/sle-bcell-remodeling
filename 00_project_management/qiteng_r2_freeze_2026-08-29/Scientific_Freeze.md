...
确认日期：2026-08-29。状态：**作者已确认科学正文冻结；Zenodo 新版本 `22151739` 与 GitHub `v1.1.0` 已公开发布并完成元数据、提交目标与文件哈希核验；旧记录 `22086892` 已按授权删除并保留公开 tombstone。**
...
正确顺序已经执行：检查账户及已有草稿，创建同一版本链的新版本并预留 DOI，构建对应冻结内容，核对科学正文和哈希，发布并验证新版本，最后使用平台支持的删除流程处理指定旧版本。旧记录 API 与 DOI 均返回 `410`，其页面保留引用、删除日期和 `retracted` 原因；新 DOI 与概念 DOI 均继续解析到 `22151739`。
...
GitHub 已创建带注释标签 `v1.1.0`，精确指向冻结内容提交 `f1859ff8498d5569a1d5027b36ed18c8b7c7536f`。公开 release 的三个资产与 Zenodo 上传文件在名称、字节数和 SHA-256 上完全相同。未删除旧 GitHub `v1.0.0` release/tag，也未改写任何科学 HOLD 或公开归档字节。
