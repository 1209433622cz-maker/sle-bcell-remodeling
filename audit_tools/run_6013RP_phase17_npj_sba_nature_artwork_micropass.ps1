...
$Run = [System.IO.Path]::GetFullPath((Join-Path $Root "phase17_v7\npj_sba_nature_artwork_micropass\20260901_role_aware_typography_refreeze"))
...
$Stems = @(
    "Manuscript_scientific_maintenance_freeze",
    "Supplementary_Information_scientific_maintenance_freeze"
)
...
try {
    if ($SkipIntegration) {
        Write-Host "[1/10] Reusing the independently verified artwork integration..."
        if (-not (Test-Path -LiteralPath (Join-Path $Run "00_NATURE_ARTWORK_MICROPASS_INTEGRATION_STATUS.json"))) {
            throw "SkipIntegration requested but the integration status is missing"
        }
    } else {
        Write-Host "[1/10] Auditing and redrawing confirmed typography outliers from frozen sources..."
        & $FigurePython ".\audit_tools\phase17_npj_sba_37_integrate_nature_artwork_micropass.py"
        if ($LASTEXITCODE -ne 0) { throw "Nature artwork integration failed" }
    }

    Write-Host "[2/10] Building manuscript and Supplementary DOCX files..."
    & $BundledPython ".\audit_tools\phase17_npj_sba_38_build_nature_artwork_documents.py"
    ...
    Write-Host "[7/10] Checking Supplementary pagination and embedded figure identity..."
    ...
    Write-Host "[9/10] Finalizing the maintenance freeze and action report..."
    & $BundledPython ".\audit_tools\phase17_npj_sba_39_finalize_nature_artwork_qa.py"
    if ($LASTEXITCODE -ne 0) { throw "Final Nature artwork QA failed" }
...
Write-Host "Nature/npj artwork typography micropass completed."
