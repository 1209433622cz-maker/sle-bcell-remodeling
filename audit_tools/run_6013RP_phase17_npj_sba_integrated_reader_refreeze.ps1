...
$Run = [System.IO.Path]::GetFullPath((Join-Path $Root "phase17_v7\npj_sba_integrated_reader_refreeze\20260901_s3_s5_reader_path_refreeze"))
...
$Stems = @(
    "Manuscript_integrated_reader_refreeze",
    "Supplementary_Information_integrated_reader_refreeze"
)
...
Push-Location $Root
try {
    Write-Host "[1/11] Verifying exact duplicates and redrawing S3/S5 from frozen Source Data..."
    & $FigurePython ".\audit_tools\phase17_npj_sba_31_integrate_reader_prune.py"
    if ($LASTEXITCODE -ne 0) { throw "Reader-path source integration failed" }

    Write-Host "[2/11] Building manuscript and Supplementary DOCX files..."
    & $BundledPython ".\audit_tools\phase17_npj_sba_32_build_integrated_reader_documents.py"
    ...
    $ReviewJobs = @(
        @{ Pdf=Join-Path $Documents ($Stems[0] + ".pdf"); Prefix="wps_main"; Pages=@(3,15,31) },
        @{ Pdf=Join-Path $Documents ($Stems[1] + ".pdf"); Prefix="wps_supp"; Pages=@(9,11) },
        @{ Pdf=Join-Path $LibreOfficeDocuments ($Stems[0] + ".pdf"); Prefix="lo_main"; Pages=@(3,15,32) },
        @{ Pdf=Join-Path $LibreOfficeDocuments ($Stems[1] + ".pdf"); Prefix="lo_supp"; Pages=@(9,11) }
    )
    ...
    Write-Host "[10/11] Finalizing the integrated-reader scientific refreeze and action report..."
    & $BundledPython ".\audit_tools\phase17_npj_sba_33_finalize_integrated_reader_qa.py"
    if ($LASTEXITCODE -ne 0) { throw "Final integrated-reader QA failed" }
...
Write-Host "Integrated reader-path and S3/S5 scientific refreeze completed."
