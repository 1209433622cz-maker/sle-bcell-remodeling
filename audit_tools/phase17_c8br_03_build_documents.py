#!/usr/bin/env python3
"""Create the Gate C8BR editable documents and package assets."""
...
def prepare_package_assets() -> dict[str, object]:
    ...
    for directory in (figures_dir, supp_figures_dir, source_dir, refs_dir, docs_dir, sensitivity_dir, reproducibility_dir):
    ...
    source_zip = PACKAGE / "additional_files" / "Additional_file_2_Figure_Source_Data_GateC8BR.zip"
    ...
    sensitivity_zip = PACKAGE / "additional_files" / "Additional_file_3_Regulator_Sensitivity_GateC8BR.zip"
    ...
    for name in (
        "author_completion_matrix_gateC8BR_2026-08-25.md",
        "journal_target_decision_gateC8BR_2026-08-25.md",
        "reporting_checklist_gateC8BR_2026-08-25.md",
    ):
        path = ROOT / "04_submission" / name
        copy_file(path, docs_dir / path.name)
    copy_file(COVER_MD, docs_dir / COVER_MD.name)
    reproducibility_files = [
        copy_file(ROOT / "audit_tools" / name, reproducibility_dir / name)
        for name in (
            "environment_gateC8BR_release_2026-08-25.yml",
            "00_create_gateC8BR_release_env.ps1",
            "phase17_c8br_00_release_smoke_test.py",
            "docx_a11y_audit.py",
            "run_6013RP_phase17_gateC8BR_release_portability_preflight.ps1",
        )
    ]
...
def main() -> None:
    ...
    outputs = [
        base.markdown_to_docx(
            MANUSCRIPT_MD,
            PACKAGE / "main_text" / "Genome_Medicine_Manuscript_GateC8BR_AUTHOR_COMPLETION_REQUIRED.docx",
            body_size=12,
            double_space=True,
            line_numbers=True,
        ...
        base.markdown_to_docx(
            SUPPLEMENT_MD,
            PACKAGE / "additional_files" / "Additional_file_1_Supplementary_Information_GateC8BR.docx",
            body_size=11,
            double_space=False,
            line_numbers=False,
        ...
        base.markdown_to_docx(
            COVER_MD,
            PACKAGE / "submission_docs" / "Genome_Medicine_Cover_Letter_GateC8BR_AUTHOR_CONFIRMATION_REQUIRED.docx",
            body_size=10.5,
            double_space=False,
            line_numbers=False,
            running_header=None,
    ...
    status = {
        ...
        "status": "PASS_GATE_C8BR_DOCUMENTS_BUILT",
    ...
    (RUN_DIR / "04_GATE_C8BR_DOCUMENT_BUILD_STATUS.json").write_text(
