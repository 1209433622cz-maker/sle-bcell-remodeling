#!/usr/bin/env python3
"""Create the Gate C8 manuscript, supplement and cover-letter DOCX files."""
...
COVER_MD = ROOT / "04_submission" / "cover_letter_genome_medicine_gateC8_AUTHOR_COMPLETION_REQUIRED_2026-08-20.md"
FIGURE_SOURCE = ROOT / "phase17_v7" / "gateC7" / "20260820_manuscript_figure_integration"
SKILL_DIR = Path(r"C:\Users\Administrator\.codex\plugins\cache\openai-primary-runtime\documents\26.819.11345\skills\documents")
...
def add_markdown_table(document: Document, block: str, body_size: float) -> None:
    ...
    for r_idx, row in enumerate(raw_rows):
        for c_idx, value in enumerate(row):
            ...
            add_inline(paragraph, value, size=body_size - 1)
            if r_idx == 0:
...
def markdown_to_docx(source: Path, output: Path, *, body_size: float, double_space: bool, line_numbers: bool, running_header: str | None, title_override: str | None = None) -> dict:
    document = Document()
    ...
    document.core_properties.comments = "Generated reproducibly from Gate C8 Markdown sources."
    ...
    for block in blocks:
        ...
        if block.startswith("## "):
            ...
            # WPS can pull a kept heading above the printable area when a large
            # table forces the heading-table pair onto a new page.  Freeze the
            # intended S4 section break explicitly for renderer-stable output.
            if heading.startswith("Supplementary Table S4 |"):
...
def prepare_package_assets() -> dict:
    ...
    for directory in (figures_dir, source_dir, refs_dir, docs_dir):
    ...
    zip_path = PACKAGE / "additional_files" / "Additional_file_2_Figure_Source_Data_GateC8.zip"
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for path in source_files + [manifest]:
            archive.write(path, arcname=path.name)

    for path in sorted((RUN_DIR / "references").glob("*")):
        if path.is_file():
            shutil.copy2(path, refs_dir / path.name)
    for source in (
        ROOT / "04_submission" / "author_completion_form_gateC8_2026-08-20.md",
        ROOT / "04_submission" / "journal_target_decision_gateC8_2026-08-20.md",
        ROOT / "04_submission" / "reporting_checklist_gateC8_2026-08-20.md",
        ROOT / "04_submission" / "cover_letter_genome_medicine_gateC8_AUTHOR_COMPLETION_REQUIRED_2026-08-20.md",
    ):
...
def main() -> None:
    if PACKAGE.exists():
        # Rebuild only the active Gate C8 package; no legacy directory is touched.
    ...
    outputs.append(
        markdown_to_docx(
            MANUSCRIPT_MD,
            PACKAGE / "main_text" / "Genome_Medicine_Manuscript_GateC8_AUTHOR_COMPLETION_REQUIRED.docx",
            body_size=12,
            double_space=True,
            line_numbers=True,
            running_header="Genome Medicine | Research | Gate C8",
            title_override="Genome Medicine manuscript Gate C8",
        )
    )
    outputs.append(
        markdown_to_docx(
            SUPPLEMENT_MD,
            PACKAGE / "additional_files" / "Additional_file_1_Supplementary_Information_GateC8.docx",
            body_size=11,
            double_space=False,
            line_numbers=False,
            running_header="Supplementary information | Gate C8",
            title_override="Supplementary information Gate C8",
        )
    )
    outputs.append(
        markdown_to_docx(
            COVER_MD,
            PACKAGE / "submission_docs" / "Genome_Medicine_Cover_Letter_GateC8_AUTHOR_CONFIRMATION_REQUIRED.docx",
            body_size=11,
            double_space=False,
            line_numbers=False,
            running_header=None,
            title_override="Genome Medicine cover letter Gate C8",
        )
    )
    ...
    (RUN_DIR / "02_GATE_C8_DOCUMENT_BUILD_STATUS.json").write_text(json.dumps(status, indent=2), encoding="utf-8")
