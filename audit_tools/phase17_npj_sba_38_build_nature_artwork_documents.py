#!/usr/bin/env python3
"""Build DOCX files for the role-aware Nature/npj artwork micropass."""
...
def patch_properties(path: Path, subject: str) -> None:
    ...
    document.core_properties.comments = (
        "Scientific-presentation maintenance freeze; role-aware typography redrawn from frozen Source Data; "
...
def main() -> None:
    ...
    manuscript_source = SOURCES / "Manuscript_nature_artwork_micropass.md"
    supplement_source = SOURCES / "Supplementary_Information_nature_artwork_micropass.md"
    manuscript = OUTPUT / "Manuscript_scientific_maintenance_freeze.docx"
    supplement = OUTPUT / "Supplementary_Information_scientific_maintenance_freeze.docx"
    ...
    patch_properties(manuscript, "Nature/npj role-aware artwork typography micropass")
    patch_properties(supplement, "Source-driven role-aware supplementary artwork typography micropass")
    ...
    checks = {
        "manuscript": {
            ...
            "internal_replication_boundary_present": "provide internal replication rather than independent replication" in main_text,
            "uncertainty_replication_boundary_present": "they are not independent replication" in main_text,
            "methodological_validation_retained": "prospective clinical validation" in main_text,
        },
        "supplement": {
            "ten_figures_embedded": len(supplementary_document.inline_shapes) == 10,
            "title_exact": TITLE in supplementary_text,
            "s3_pruned_title_present": "Fine-state failure and transition structure" in supplementary_text,
            "s5_owner_sentence_present": "owned by Fig. 3b and are not repeated here" in supplementary_text,
            "selected_figure_source_map_title": "Selected figure source-data map" in supplementary_text,
        },
    }
    failed = [
        f"{group}.{name}"
    ...
    if failed:
        raise RuntimeError(f"Nature artwork document checks failed: {failed}")

    status = {
        "created_at": datetime.now().astimezone().isoformat(timespec="seconds"),
        "status": "PASS_NATURE_ARTWORK_MICROPASS_DOCX_BUILT_DUAL_RENDER_REQUIRED",
