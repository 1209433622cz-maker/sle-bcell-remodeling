#!/usr/bin/env python3
"""Create the Gate C8R manuscript, supplement and cover-letter DOCX files."""
...
COVER_MD = ROOT / "04_submission" / "cover_letter_genome_medicine_gateC8R_AUTHOR_COMPLETION_REQUIRED_2026-08-20.md"
...
def markdown_to_docx(source: Path, output: Path, *, body_size: float, double_space: bool, line_numbers: bool, running_header: str | None, title_override: str | None = None) -> dict:
    ...
    document.core_properties.comments = "Generated reproducibly from Gate C8R Markdown sources."
...
def prepare_package_assets() -> dict:
    ...
    for directory in (figures_dir, source_dir, refs_dir, docs_dir, sensitivity_dir):
    ...
    zip_path = PACKAGE / "additional_files" / "Additional_file_2_Figure_Source_Data_GateC8R.zip"
    ...
    for source in (
        ROOT / "04_submission" / "author_completion_form_gateC8R_2026-08-20.md",
        ROOT / "04_submission" / "journal_target_decision_gateC8R_2026-08-20.md",
        ROOT / "04_submission" / "reporting_checklist_gateC8R_2026-08-20.md",
        ROOT / "04_submission" / "cover_letter_genome_medicine_gateC8R_AUTHOR_COMPLETION_REQUIRED_2026-08-20.md",
    ):
...
def main() -> None:
    if PACKAGE.exists():
        # Rebuild only the active Gate C8R package; no legacy directory is touched.
    ...
    outputs.append(
        markdown_to_docx(
            MANUSCRIPT_MD,
            PACKAGE / "main_text" / "Genome_Medicine_Manuscript_GateC8R_AUTHOR_COMPLETION_REQUIRED.docx",
            body_size=12,
            double_space=True,
            line_numbers=True,
            running_header="Genome Medicine | Research | Gate C8R",
            title_override="Genome Medicine manuscript Gate C8R",
        )
    )
    outputs.append(
        markdown_to_docx(
            SUPPLEMENT_MD,
            PACKAGE / "additional_files" / "Additional_file_1_Supplementary_Information_GateC8R.docx",
            body_size=11,
            double_space=False,
            line_numbers=False,
            running_header="Supplementary information | Gate C8R",
            title_override="Supplementary information Gate C8R",
        )
    )
    outputs.append(
        markdown_to_docx(
            COVER_MD,
            PACKAGE / "submission_docs" / "Genome_Medicine_Cover_Letter_GateC8R_AUTHOR_CONFIRMATION_REQUIRED.docx",
            body_size=10.5,
            double_space=False,
            line_numbers=False,
            running_header=None,
            title_override="Genome Medicine cover letter Gate C8R",
        )
    )
    ...
    (RUN_DIR / "06_GATE_C8R_DOCUMENT_BUILD_STATUS.json").write_text(json.dumps(status, indent=2), encoding="utf-8")
